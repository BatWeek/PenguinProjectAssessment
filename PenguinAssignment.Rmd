...
species_colours <- c("Adelie" = "darkorange", "Chinstrap" = "purple", 
                     "Gentoo" = "cyan4")
...
species_shape <- c("Adelie" = 15, "Chinstrap" = 16, "Gentoo" = 17)
...
#Loads the "PenguinFunctions" file within the "functions" folder, containing a function to save graphics as .SVG files. This function is easy to find, should it need to be modified.
...
The data were separated based on penguin species, and each species plotted on its own plot. The data were plotted as histograms to see if they followed a normal distribution, to determine whether a parametric or a non-parametric test would be appropriate. These histograms are saved as an additional figure. For adelie and gentoo penguins, both culmen length and culmen depth appears to be sufficiently consistent with a normal distribution to use parametric tests, and for chinstrap penguins, consistency of the culmen lengths with the normal distribution was found after a Shapiro-Wilk test. The Pearson's r correlation coefficient was used to characterise the relationship between variables of all three species.
...
Adelie_length_hist <- ggplot(data = Adelie_data, aes(x = culmen_length_mm)) + 
  ...
  labs(x = "Culmen length (mm)", y = "Frequency", 
...
Adelie_depth_hist <- ggplot(data = Adelie_data, aes(x = culmen_depth_mm)) + 
  ...
  labs(x = "Culmen depth (mm)", y = "Frequency",
...
#The Chinstrap culmen length histogram seems like it may not be normal, so a Shapiro-Wilk test is conducted on this data to clarify this. The it has a large enough p-value (0.1941 > 0.05) to still be considered normally distributed.
